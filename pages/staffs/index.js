...
export default function Staff({ serviceDomain, microcmsApiKey }) {
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((content) => {
        setStaff(content);
        getReservations(client, `staff[equals]${staff?.id}`).then(
          (_reservations) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                {dates.map((workday) => {
                  return (
                    <TableCell key={`week-${workday.toISOString()}`}>
                      {workday.getMonth() + 1}/{workday.getDate()}(
                      {weekJp[workday.getDay()]})
                    </TableCell>
                  );
                })}
              </TableRow>
                ...
                return (
                  <TableRow key={`staff-${hour}`}>
                    ...
                    {dates.map((workday) => {
                      ...
                      const isWorking = isIncludeWorkday(
                        workdays,
                        workday,
                        hour
                      );
                      ...
                      return isWorking ? (
                        reservation ? (
                          <TableCell
                            key={`week-${workday.toISOString()}-${hour}`}
                            sx={{ bgcolor: red[500], color: grey[50] }}
                            align="center"
                          >
                            {
                          ...
                          >
                            <Button
                              variant="text"
                              onClick={() => {
                                reserve(_date, staff.id, profile);
                              }}
                            >
                              予約
                            </Button>
